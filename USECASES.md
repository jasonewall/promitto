...
The benefit of using `settled()` is very situtational and there are likely scenarios where it is better/simpler to `await` the end of a chain. There are also other consistent/effective solutions depending on the situation. e.g. Component tests have `waitFor()`. It is just meant to be another tool in the box for when nothing else is available.
